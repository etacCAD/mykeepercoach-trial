...
exports.buildWebSessionPrompt = buildWebSessionPrompt;
exports.parseGeminiJson = parseGeminiJson;
exports.buildVideoPartsFromSignedUrls = buildVideoPartsFromSignedUrls;
exports.analyzeWebSession = analyzeWebSession;
/**
...
// ── Prompt Builder ───────────────────────────────────────────────────────────
function buildWebSessionPrompt(params) {
...
async function buildVideoPartsFromSignedUrls(bucket, storagePaths) {
    return Promise.all(storagePaths.map(async (p) => {
 ...
 */
async function analyzeWebSession({ sessionRef, sessionData, bucket, ageGroup, }) {
    try {
        ...
        if (allVideoPaths.length === 0)
            throw new Error("No videos in session");
...
//# sourceMappingURL=geminiAnalysis.js.map